 ...
 * Git fields are included only when the supplied root is itself a Git checkout,
 * so an unpacked plugin remains fully identifiable by its content fingerprint.
 */
export function inspectBuildIdentity(projectRoot, options = {}) {
  ...
  return Object.freeze({
    ...
    build_fingerprint: computeBuildFingerprint(root, {
      ...options,
      packageMetadata,
    }),
    ...(git.commit ? { git_commit: git.commit } : {}),
    ...(typeof git.dirty === "boolean" ? { git_dirty: git.dirty } : {}),
  });
}
