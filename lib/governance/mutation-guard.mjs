...
export function withGovernedMutationBatch(requests, callback) {
  ...
  const invoke = (index) => index === requests.length
    ? callback()
    : withGovernedMutation(requests[index], () => invoke(index + 1));
  return invoke(0);
}
...
export function consumeBootstrapMutationGrant(grant, callback) {
  ...
  const lifetime = { active: true, exactMutations: binding.exact_mutations };
...
function decideMutation(context, request) {
  ...
  try {
    ...
    if (bootstrap) {
      ...
      const allowed = bootstrap.exactMutations.some((candidate) =>
        computeStableHash(candidate) === computeStableHash(exact));
      return allowed
        ? immutableJson({
            allowed: true,
            request,
