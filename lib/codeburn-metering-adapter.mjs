...
export async function detectCodeBurn(options = {}) {
  ...
  try {
    const result = await execution.executor(execution.executable, ["--version"], execution.process_options);
...
/**
 * Execute a bounded CodeBurn 0.9.x JSON report. No raw argv or shell option is
 * accepted. The returned report is validated before it crosses the adapter.
 */
export async function executeCodeBurnReport(input, options = {}) {
  ...
  let result;
  try {
    result = await execution.executor(execution.executable, argv, execution.process_options);
  } catch (error) {
