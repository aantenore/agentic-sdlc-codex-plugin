...
function mutateIdentity(operation, targetPath, callback) {
  ...
  return gateway({ operation, path: path.resolve(targetPath) }, callback);
}

function renameIdentityPath(sourcePath, targetPath) {
  return mutateIdentity("path.rename.source", sourcePath, () =>
    mutateIdentity("path.rename.target", targetPath, () => fs.renameSync(sourcePath, targetPath)));
}

function linkIdentityPath(sourcePath, targetPath) {
  return mutateIdentity("path.link.source", sourcePath, () =>
    mutateIdentity("path.link.target", targetPath, () => fs.linkSync(sourcePath, targetPath)));
}
