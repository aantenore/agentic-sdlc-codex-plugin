...
function renameIdentityPath(sourcePath, targetPath) {
...
function linkIdentityPath(sourcePath, targetPath) {
...
function createIdentityExecutionDescriptor({ id, planHash, root, sdlcRoot, writes }) {
  const nonce = sha256(`identity-migration-execution:v1\0${id}`).slice(0, 24);
...
function acquireLock(lockPath, migrationId, planHash, execution) {
  ...
  try {
    ...
    fs.closeSync(descriptor);
    descriptor = undefined;
    linkIdentityPath(temporaryPath, lockPath);
    published = true;
    mutateIdentity("file.remove", temporaryPath, () => fs.rmSync(temporaryPath));
...
function atomicWrite(filePath, content, projectRoot, execution = null) {
  ...
  try {
    ...
    mutateIdentity("path.chmod", tempPath, () => fs.chmodSync(tempPath, 0o600));
    assertDirectoryIdentity(path.dirname(resolved), parentIdentity, projectRoot);
    renameIdentityPath(tempPath, resolved);
  } finally {
    mutateIdentity("file.remove", tempPath, () => fs.rmSync(tempPath, { force: true }));
  }
}
