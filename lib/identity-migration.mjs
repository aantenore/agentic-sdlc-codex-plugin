...
function renameIdentityPath(sourcePath, targetPath, expectedSourceIdentity = null) {
...
function assertOwnedTemporaryFile(temporaryPath, expectedIdentity) {
  if (!expectedIdentity) {
...
function atomicWrite(filePath, content, projectRoot, execution = null) {
  ...
  try {
    ...
    descriptor = undefined;
    assertDirectoryIdentity(path.dirname(resolved), parentIdentity, projectRoot);
    assertOwnedTemporaryFile(tempPath, tempIdentity);
    renameIdentityPath(tempPath, resolved, tempIdentity);
  } finally {
    if (descriptor !== undefined) {
      try { fs.closeSync(descriptor); } catch {}
    }
