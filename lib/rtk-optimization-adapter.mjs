...
function resolveWindowsNativePathCommand(options) {
  ...
  for (const directory of windowsSearchDirectories(cwd, environment, pathModule)) {
...
function resolveWindowsPathCommand(options) {
  ...
  const extension = pathModule.extname(token).toLowerCase();
  const searchDirectories = windowsSearchDirectories(cwd, environment, pathModule);
  const candidateExtensions = extension
...
function windowsSearchDirectories(cwd, environment, pathModule) {
  ...
  const configured = typeof pathValue === "string"
    ...
    : [];
  // Search only directories explicitly named by PATH. Implicit cwd lookup
  // would let a project-local shim shadow the host command unexpectedly.
  const directories = configured
    .map((entry) => String(entry || "").trim().replace(/^"(.*)"$/u, "$1"))
    .filter(Boolean)
    .map((entry) => pathModule.resolve(cwd, entry));
