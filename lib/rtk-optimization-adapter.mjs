...
export function routeRtkCommand(command, options = {}) {
  ...
  const executionCommand = buildExecutionCommand(executableName, normalizedCommand, platform);
...
function buildExecutionCommand(executableName, command, platform = process.platform) {
  const executable = resolvePlatformExecutable(command[0], executableName, platform);
  if (executableName === "rg") {
    return Object.freeze([executable, "--no-config", ...command.slice(1)]);
  }
  ...
  return Object.freeze([executable, ...command.slice(1)]);
}
...
function resolvePlatformExecutable(token, executableName, platform) {
  if (
    platform === "win32"
    && !/\.(?:exe|cmd|bat)$/iu.test(token)
    && ["npm", "pnpm", "yarn", "jest", "vitest"].includes(executableName)
  ) {
    return `${token}.cmd`;
  }
  return token;
...
function normalizePlatform(value) {
  const platform = value === undefined ? process.platform : String(value).trim().toLowerCase();
