...
const OPTION_LIBRARY = deepFreeze({
  ...
  "template-dir": option(
    "template-dir",
    "path",
    "Initialize from a local template overlay. It must contain sdlc-config.json; missing template assets use bundled fallbacks. The resulting configuration is copied into the project and pinned.",
    "Inizializza da un overlay locale di template. Deve contenere sdlc-config.json; gli asset mancanti usano i fallback inclusi. La configurazione risultante viene copiata e bloccata nel progetto.",
  ),
...
const COMMANDS = [
  ...
  C("autonomy delivery propose", "Choose how independently I should work for this one exact delivery. Pull requests and local releases always use separate choices.", "Scegli quanta autonomia devo avere per questa singola consegna esatta. Pull request e rilasci locali usano sempre scelte separate.", {
    ...
    examples: [
      ...
      "autonomy delivery propose --id AUT-DATA-009 --delivery LOCAL-DATA-009 --kind local_release --story ST-001 --contract contract-ST-001-release --requirement REQ-001 --level checkpointed --target-root /absolute/local-data --write-path app --write-path data --smoke-cwd app --smoke-test '[\"npm\",\"run\",\"smoke:local\"]' --allow-action build.local --allow-action test.run --allow-action data.migrate --allow-action data.rollback --allow-action release.local --data-target data/store.json --data-scope customers.v2 --migration-preview evidence/migration-preview.json --backup-path data/store.backup.json --rollback \"Restore data/store.json from the exact backup\"",
    ],
  }),
