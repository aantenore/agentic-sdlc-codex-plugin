...
const OPTION_LIBRARY = deepFreeze({
  ...
  "no-open": option("no-open", null, "Do not open a browser window.", "Non apre una finestra del browser."),
  ...
  level: option("level", "supervised|checkpointed|bounded-autonomous", "Choose the requested independence for this delivery.", "Scegli l’autonomia richiesta per questa consegna.", { values: ["bounded-autonomous", "checkpointed", "supervised"] }),
...
function command(path, en, it, {
  ...
  return { path, description: text(en, it), effect, protection: EFFECTS[effect], options, positionals, usage, examples, aliases };
}
...
const COMMANDS = [
  ...
  C("observe", "Open the local Change Observatory.", "Apre il Change Observatory locale.", { options: ["host", "port", "no-open"], examples: ["observe", "observe --no-open --json"] }),
  ...
  C("autonomy delivery propose", "Propose how independently one exact pull request or local release may proceed.", "Propone quanta autonomia dare a una specifica pull request o a un rilascio locale.", {
    effect: "plan",
    options: [
      required("id"),
      required("delivery"),
  ...
  C("autonomy delivery status", "Explain what this delivery may actually do now.", "Spiega cosa può realmente fare questa consegna adesso.", { options: ["id"] }),
  C("autonomy delivery explain", "Export a human-readable explanation of one delivery choice.", "Esporta una spiegazione leggibile di una scelta di consegna.", { options: ["id", "phase", "out"] }),
  ...
  C("contract approve", "Approve one exact implementation agreement.", "Approva un accordo di implementazione esatto.", { effect: "protected", options: ["id", "summary", "authorization"] }),
  C("story create", "Create one deliverable unit of work linked to a requirement.", "Crea un’attività consegnabile collegata a un requisito.", { effect: "local", options: ["id", "requirement"] }),
  C("story claim", "Assign one unit of work to one agent and branch.", "Assegna un’attività a un agente e a un branch.", { effect: "local", options: ["id"] }),
  ...
  C("task start", "Confirm that one unit of work can begin under its agreed implementation terms and limits.", "Conferma che un’attività può iniziare secondo i termini e i limiti di implementazione concordati.", { effect: "protected", options: ["story", "phase", "contract", "delivery-profile", "authorization"] }),
  ...
  C("output resolve", "Find the agreed output format for one delivery.", "Trova il formato di output concordato per una consegna.", { options: ["story"] }),
  ...
  C("migration identity", "Plan, apply, or recover an attributed identity migration.", "Pianifica, applica o recupera una migrazione delle identità attribuite.", { effect: "protected", options: ["apply", "plan-hash"] }),
