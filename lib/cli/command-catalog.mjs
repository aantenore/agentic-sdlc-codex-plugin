...
const OPTION_LIBRARY = deepFreeze({
  ...
  "template-dir": option(
    "template-dir",
    "path",
    "Initialize this project from an exact local template directory. The resulting configuration is copied into the project and pinned; later commands do not inherit template changes.",
    "Inizializza il progetto da una cartella locale di template esatta. La configurazione risultante viene copiata e bloccata nel progetto; i comandi successivi non ereditano modifiche ai template.",
  ),
...
const COMMANDS = [
  ...
  C("config migrate", "Plan or apply a configuration migration or autonomy rollout-mode change.", "Pianifica o applica una migrazione della configurazione o una modifica alla modalità di adozione dell’autonomia.", {
    effect: "local",
    options: ["autonomy-mode", "apply", "plan-hash"],
    usage: "config migrate [--autonomy-mode <mode>] [--apply --plan-hash <sha256>]",
    examples: [
      "config migrate",
