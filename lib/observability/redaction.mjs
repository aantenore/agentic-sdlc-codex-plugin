...
export const BUILTIN_OPERATIONAL_SECRET_PATTERNS = Object.freeze([
  Object.freeze({ name: "secret_assignment", pattern: /\b(?:access[_-]?token|api[_-]?key|authorization|client[_-]?secret|password|refresh[_-]?token|secret|token)\s*[:=]\s*["']?[A-Za-z0-9._~+/=-]{8,}["']?/giu }),
  ...
  Object.freeze({ name: "bearer_credential", pattern: /\bBearer\s+[A-Za-z0-9._~+/=-]{16,}/giu }),
...
export const BUILTIN_OPERATIONAL_PII_PATTERNS = Object.freeze([
  Object.freeze({ name: "email", pattern: /\b[^\s@]+@[^\s@]+\.[^\s@]+\b/giu }),
]);
