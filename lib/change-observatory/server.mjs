...
import { resolveObservatoryConfiguration } from "./configuration.mjs";
...
export async function createObservatoryRequestHandler(options = {}) {
  ...
  const observabilityConfiguration = await resolveObservatoryConfiguration(projectRoot, {
  ...
  const handler = async function observatoryRequestHandler(request, response) {
    ...
    try {
      ...
      return sendError(request, response, error, requestState.context);
    }
  };
...
export async function startObservatoryServer(options = {}) {
  ...
  const server = http.createServer((request, response) => {
    handler(request, response).catch((error) => {
      ...
      return sendError(request, response, error, context);
    });
  });
