...
// Keep warm-cache validation below the point where repeated lstat/opendir
// batches make V8 expand its old-space heap on enterprise-sized trees. Eight
// workers retain parallel I/O without changing the bounded validation model.
...
export function createObservatoryModelCache(options = {}) {
  ...
  async function refresh() {
    ...
    if (cached && validationSnapshot) {
      ...
      if (!valid) validationSnapshot = null;
      await assertDirectoryIdentity(projectIdentity);
      if (valid) {
        emit("fast_hit");
        return cached;
...
async function validateDirectoryObservation(observation, entryBudget) {
  ...
  const directoryRead = Array.isArray(observation.snapshot)
    ? readAndMatchDirectDirectorySnapshot(observation.path, observation.snapshot, {
...
async function readDirectoryEntries(directory, {
  ...
  try {
    // Keep traversal streaming and bounded without allocating one Promise per
    // directory entry. The async iterator form caused the warm validation path
    // to expand V8's heap substantially on enterprise-sized trees even though
    // each retained collection was already bounded.
...
function readAndMatchDirectDirectorySnapshot(directory, snapshot, {
  aggregateBudget = null,
  maxEntries = MAX_DIRECTORY_ENTRIES,
} = {}) {
  let handle;
