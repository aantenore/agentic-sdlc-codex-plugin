...
async function validateDirectoryObservation(observation) {
  ...
  return directoryRead.readable === observation.readable
    && (!directoryRead.readable || directoryEntriesMatchSnapshot(
...
async function walkCanonicalDirectory(state, directory, relativeDirectory, depth) {
  ...
  const directorySignature = createStatSignature(before);
  const observation = {
    path: directory,
    readable: null,
...
async function recordCanonicalFile(state, absolute, relative, initialStats) {
  ...
  if (previous && statMatchesSignature(initialStats, previous.signature)) {
    ...
    read = await hashStableFile(absolute, signature);
  }
  if (!read.readable) {
...
async function hashStableFile(absolute, expectedSignature) {
  let handle;
  try {
...
function createStatSignature(stats) {
  // ctime is not a portable cache key: Windows can advance it while the
  // underlying file identity and bytes are unchanged. The remaining fields
  // still bind replacement, type, permissions, size, and content-time drift.
  return {
    dev: stats.dev,
    ino: stats.ino,
    mode: stats.mode,
...
function statMatchesSignature(stats, signature) {
  return stats.dev === signature.dev
    ...
    && stats.size === signature.size
    && stats.mtimeNs === signature.mtimeNs;
}
