...
export async function buildObservatoryViewModel(projectRoot, options = {}) {
  ...
  const autonomyRecords = records
    .filter((record) => [
      "requirement-execution-profile",
      "delivery-execution-profile",
      "autonomy-decision",
    ].includes(record.kind) && isRecordObject(record))
    .map((record) => itemFromRecord(record, { limits, diagnostics }));
  const decisions = [
    ...records
...
function autonomyDisplayDefaults(record, data) {
  ...
  if (record.kind === "requirement-execution-profile") {
    const requirementId = typeof data.requirement_ref?.id === "string"
      ? data.requirement_ref.id
      : null;
    return {
      title: requirementId ? `Autonomy ceiling for ${requirementId}` : `Requirement autonomy ${data.id ?? "profile"}`,
      summary: data.autonomy_ceiling
        ? `Maximum ${data.autonomy_ceiling} autonomy${requirementId ? ` for requirement ${requirementId}` : ""}.`
        : null,
      status: data.status ?? null,
    };
  }
  if (record.kind === "delivery-execution-profile") {
    const deliveryKind = data.delivery_kind === "pull_request"
      ? "pull request"
      : data.delivery_kind === "local_release" ? "local release" : "delivery";
    return {
      title: data.delivery_id
        ? `Autonomy for ${deliveryKind} ${data.delivery_id}`
        : `Delivery autonomy ${data.id ?? "profile"}`,
      summary: data.requested_level
        ? `Selected ${data.requested_level} autonomy for ${deliveryKind}${data.delivery_id ? ` ${data.delivery_id}` : ""}.`
        : null,
      status: data.status ?? null,
    };
  }
  if (record.kind === "autonomy-decision") {
    const deliveryId = typeof data.delivery?.id === "string" ? data.delivery.id : null;
    const reasons = asArray(data.reason_codes)
      .filter((reason) => typeof reason === "string" && reason.trim() !== "")
      .slice(0, 20);
    const result = [
      data.requested_level ? `requested ${data.requested_level}` : null,
      data.effective_level ? `effective ${data.effective_level}` : null,
      data.execution_status ? `status ${data.execution_status}` : null,
    ].filter(Boolean).join("; ");
    return {
      title: deliveryId
        ? `Autonomy decision for ${deliveryId}`
        : `Autonomy decision ${data.id ?? "record"}`,
      summary: `${result || "Autonomy was evaluated"}${reasons.length > 0 ? `; reasons: ${reasons.join(", ")}` : ""}.`,
...
function normalizeIterationDossiers(records, iterations, limits, diagnostics) {
  ...
  const place = (context, record, via, refs, { anchor = false } = {}) => {
    ...
    const recordedAutonomyStoryIds = record.kind === "delivery-execution-profile"
      ...
      ? normalizedStringList(
          asArray(record.data.story_refs).map((ref) => isPlainObject(ref) ? ref.id : null),
          limits,
        )
      : [];
  ...
  for (const context of contexts) {
    ...
    for (const record of records) {
      if (record.kind !== "delivery-execution-profile" || !isRecordObject(record)) continue;
      const storyRefs = asArray(record.data.story_refs);
      for (let index = 0; index < storyRefs.length; index += 1) {
        if (!isPlainObject(storyRefs[index]) || storyRefs[index].id !== storyId) continue;
        place(
          context,
          record,
          "story_ref",
          [{ ...sourceRef(record), pointer: `/story_refs/${index}` }],
          { anchor: true },
        );
      }
    }
    ...
    for (const placement of autonomyAnchors) {
      ...
      if (record.kind === "requirement") {
        const profileId = boundedText(record.data.autonomy_profile_id, limits.maxTextChars).value;
        if (profileId) {
      ...
      if (record.kind === "contract") {
        const requirementProfileRefs = asArray(record.data.requirement_execution_profile_refs);
        for (let index = 0; index < requirementProfileRefs.length; index += 1) {
          ...
          const profileId = isPlainObject(profileRef)
            ? boundedText(profileRef.id, limits.maxTextChars).value
            : null;
        ...
        const deliveryProfileId = boundedText(
      ...
      if (isPlainObject(deliveryProfileRef)) {
        const profileId = boundedText(deliveryProfileRef.id, limits.maxTextChars).value;
        if (profileId) {
      ...
      if (isPlainObject(decisionRef)) {
        const decisionId = boundedText(decisionRef.id, limits.maxTextChars).value;
        if (decisionId) {
    ...
    const linkedDeliveryProfileIds = new Set(
      [...context.placements.values()]
        ...
        .map((placement) => placement.record.data?.id)
        .filter(Boolean),
    );
    for (const record of records) {
      if (record.kind !== "autonomy-decision" || !isRecordObject(record)) continue;
      const profileId = boundedText(record.data.delivery?.profile_id, limits.maxTextChars).value;
      if (!profileId || !linkedDeliveryProfileIds.has(profileId)) continue;
      place(
        context,
        record,
