...
class DiagnosticCollector {
  ...
  add({ code, severity = "warning", message, provenance = "inferred", sourceRefs = [] }) {
    const normalizedRefs = sourceRefs.map(normalizeSourceRef);
...
function buildExplanation(data, narrative, privateReasoningPresent, fallbackSummary, refs, limits) {
  if (narrative) {
    ...
    const rationale = boundedText(
      narrative.rationale_summary ?? narrative.rationale,
      limits.maxTextChars,
    );
...
function containsPrivateReasoningSignal(value) {
  if (Array.isArray(value)) return value.some(containsPrivateReasoningSignal);
  if (!isPlainObject(value)) return false;
  return Object.entries(value).some(([key, item]) => {
    const normalized = normalizeSensitiveKey(key);
    if (PRIVATE_REASONING_KEYS.has(normalized)) return true;
    if (PRIVATE_REASONING_FLAG_KEYS.has(normalized) && item === true) return true;
    return containsPrivateReasoningSignal(item);
  });
}
