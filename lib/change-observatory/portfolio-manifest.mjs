...
async function resolvePortfolioRoot(value) {
  ...
  try {
    const lexicalStats = await fs.lstat(lexical);
    if (lexicalStats.isSymbolicLink()) {
      throw new ObservatoryPathError("portfolio_root_symlink", "The portfolio root may not be a symlink", 403);
    }
  } catch (error) {
  ...
  if (root !== lexical) {
    throw new ObservatoryPathError("portfolio_root_symlink", "The portfolio root path may not contain symlinks", 403);
  }
  return root;
}
