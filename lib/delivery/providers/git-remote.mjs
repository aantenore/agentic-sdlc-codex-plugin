...
export function createGitRemoteProvider({ commandRunner = defaultCommandRunner } = {}) {
  ...
  return Object.freeze({
    ...
    observePrecondition(operation) {
      ...
      const subject = normalizeGitPushSubject(operation.subject);
    ...
    verifyCompletion(operation, { precondition_receipt: preconditionReceipt }) {
      ...
      if (observedSha !== subject.source_sha) {
        throw new DeliveryProviderError(
          `git.push completion is not proven: ${subject.destination_ref} resolves to ${observedSha}, expected ${subject.source_sha}`,
...
function normalizeGitPushSubject(subject) {
  ...
  const cwd = requireNonEmptyString(subject.cwd, "git.push.subject.cwd");
  if (!path.isAbsolute(cwd) || cwd.includes("\0")) {
    throw new DeliveryProviderError("git.push subject.cwd must be an absolute path", "provider_operation_invalid");
  }
