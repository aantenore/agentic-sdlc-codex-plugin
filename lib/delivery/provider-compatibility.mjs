...
/**
 * Resolve the provider bound to an action without inventing a fallback. V2
 * profiles must carry their exact binding; only historical v1 profiles use the
 * documented in-memory compatibility mapping above.
 */
export function providerBindingForAction(profile, action) {
  if (!isPlainRecord(profile)) {
    throw new DeliveryProviderError("Delivery profile must be an object", "provider_binding_invalid");
  }
  const normalizedAction = requireNonEmptyString(action, "delivery_action").toLowerCase();
  if (profile.schema_version === "delivery-execution-profile:v1") {
    return legacyProviderForAction(profile, normalizedAction);
  }
  if (profile.schema_version !== "delivery-execution-profile:v2") {
    throw new DeliveryProviderError(
      `Unsupported delivery profile schema '${profile.schema_version || "missing"}'`,
      "provider_binding_invalid",
    );
  }
  if (!Array.isArray(profile.provider_bindings)) {
    throw new DeliveryProviderError("Delivery profile has no explicit provider bindings", "provider_binding_invalid");
  }
  const matches = profile.provider_bindings.filter((binding) => binding?.action === normalizedAction);
  if (matches.length === 0) return null;
  if (matches.length !== 1) {
    throw new DeliveryProviderError(
      `Delivery action '${normalizedAction}' has an ambiguous provider binding`,
      "provider_binding_invalid",
    );
  }
  const providerId = requireNonEmptyString(matches[0].provider_id, "provider_binding.provider_id").toLowerCase();
  if (!/^[a-z0-9][a-z0-9._-]*$/u.test(providerId)) {
    throw new DeliveryProviderError("Delivery profile provider id is invalid", "provider_binding_invalid");
  }
  return immutableJson({
