...
export function createWorkflowTransition(input, options = {}) {
  ...
  const canonicalEvidence = normalizeCanonicalEvidenceForInstance(options.canonical_evidence, instance);
...
export function replayWorkflowEvents(input, options = {}) {
  ...
  for (let index = 0; index < events.length; index += 1) {
    ...
    const canonicalGuardResults = Array.isArray(event.guard_results)
      ? event.guard_results.filter((result) =>
          Object.hasOwn(CANONICAL_WORKFLOW_GUARD_CHECKS, result?.guard_id))
      : [];
    if (canonicalGuardResults.length > 0 || event.canonical_evidence !== undefined) {
      ...
      if (!evidenceValidation.valid) {
        ...
        for (const result of canonicalGuardResults) {
          const checkId = CANONICAL_WORKFLOW_GUARD_CHECKS[result.guard_id];
          if (
            result.allowed !== true
            || event.canonical_evidence.checks?.[checkId]?.satisfied !== true
          ) {
            errors.push(`event ${sequence} canonical guard '${result.guard_id}' lacks satisfied evidence`);
          }
        }
      }
...
export function evaluateWorkflowGuards(
  ...
  const normalizedCanonicalEvidence = normalizeCanonicalEvidence(canonicalEvidence);
...
function normalizeCanonicalEvidenceForInstance(evidence, instance) {
  const storyId = instance?.metadata?.governance_binding?.story_id;
