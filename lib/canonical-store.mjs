...
export function openCanonicalStore({ root = process.cwd(), onMetric = null } = {}) {
  ...
  const loadBuffer = (input) => {
    ...
    try {
      ...
      const completed = fileIdentity(fs.fstatSync(descriptor));
      if (!sameFileIdentity(opened, completed) || completed.size !== buffer.length) {
        throw new CanonicalStoreError(
          "file_changed",
      ...
      assertRootIdentity();
      const value = { buffer, identity: completed, sha256: null };
  ...
  const readJson = (input) => {
    ...
    const resolved = resolveCandidate(input, { mustExist: true, fileOnly: true });
    const value = loadBuffer(resolved.absolute);
    try {
      ...
      throw new CanonicalStoreError(
        "invalid_json",
        `Unable to parse canonical JSON ${resolved.key}: ${error.message}`,
        { cause: error },
      );
    }
  };
...
function verifyOpenFile(descriptor, filePath, parentIdentity) {
  ...
  const pathEntry = fs.lstatSync(filePath);
  if (
