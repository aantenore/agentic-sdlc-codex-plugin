...
export function validateReleaseArtifactPolicy(policy) {
  ...
  const packageKeys = ["name", "archive_root", "tag_prefix", "allowed_top_level", "required_files", "plugin_manifest", "installer", "bin"];
...
function verifyArchiveMetadata(archive, policy, expectedTag) {
  ...
  for (const target of Object.values(policy.package.bin)) {
    ...
    if ((binEntry.mode & 0o111) === 0) fail("BIN_TARGET_NOT_EXECUTABLE", `package bin target is not executable: ${relative}`);
  }
...
function findExecutable(command) {
  if (path.isAbsolute(command) || command.includes(path.sep) || (path.sep === "\\" && command.includes("/"))) {
    if (!existsSync(command)) fail("COMMAND_NOT_FOUND", `required command was not found: ${command}`);
...
function resolveNpmCli(configured) {
  ...
  try {
    candidates.push(findExecutable(process.platform === "win32" ? "npm.cmd" : "npm"));
  } catch (error) {
...
function snapshotTree(root) {
  ...
  const visit = (current, relative) => {
    ...
    if (stat.isSymbolicLink()) fail("SMOKE_HOME_LINK", `smoke HOME contains a symbolic link: ${relative}`);
    if (stat.isDirectory()) {
...
function runInstalledPackageSmoke({ archiveBytes, npmCliPath, packageName, pythonExecutable }) {
  ...
  try {
    ...
    const installedBin = path.join(installedRoot, "bin", "agentic-sdlc.mjs");
    if (!existsSync(installedBin) || !lstatSync(installedBin).isFile()) {
      fail("INSTALLED_BIN_MISSING", "installed package does not contain its CLI entry point");
    }
    runCommand(process.execPath, [installedBin, "--help"], { cwd: installRoot, env: environment }, "installed CLI help");
    const doctor = runCommand(
      process.execPath,
      [installedBin, "doctor", "--root", doctorRoot, "--json"],
    ...
    const beforePlan = snapshotTree(smokeHome);
    const python = findExecutable(pythonExecutable || process.env.PYTHON || (process.platform === "win32" ? "python" : "python3"));
    ...
    const planPayload = parseStrictJson(Buffer.from(plan.stdout), "installer plan JSON output");
    const afterPlan = snapshotTree(smokeHome);
    if (canonicalJson(beforePlan) !== canonicalJson(afterPlan)) {
      fail("INSTALLER_PLAN_WROTE_FILES", "installer plan changed the isolated HOME directory");
    }
    ...
    const doctorChecks = doctorPayload.checks
