...
export function openCanonicalQuerySession({
  ...
  const captureSnapshot = (resolved) => {
    if (typeof canonicalStore.snapshot === "function") {
      return canonicalStore.snapshot(resolved.projectPath);
    }
...
function immutableParsedJson(value) {
  ...
  for (const key of Object.keys(value).sort(comparePortableStrings)) {
    ...
    // Ordinary assignment avoids a costly descriptor allocation for every
    // parsed property. __proto__ alone needs an own data property so untrusted
    // canonical JSON cannot mutate the result prototype.
    if (key === "__proto__") {
