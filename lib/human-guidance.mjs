...
const CATALOG = Object.freeze({
  en: Object.freeze({
    ...
    "delivery.proposal.boundary.local": "This is for project {project}. The destination is the local folder {target_root}. I may change only {write_paths}, and the smoke test runs from {smoke_cwd}. Its explicit entrypoint must stay inside the artifact; shells, dispatchers, inline code, and ambiguous loaders are rejected. Supported hosts isolate networking differently, so a portable check must not depend on listeners or connections; test an API handler in-process instead. The runner denies file writes and external or host network access, but it can read files available to your account and does not attest transitive imports: use reviewed project code that does not load ungoverned host paths.",
