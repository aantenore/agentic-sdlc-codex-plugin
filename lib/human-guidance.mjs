...
const CATALOG = Object.freeze({
  en: Object.freeze({
    ...
    "delivery.protection.common": "Deployment, production access, secrets, changes outside the approved files, and any later pull request or local release need a separate decision.",
    ...
    "delivery.proposal.decision": "Approve or change this one-delivery choice after checking the project or local folder, destination, files I may change, review moments, and expiry.",
    "delivery.proposal.next": "If the limits are correct, approve this choice. Otherwise edit them before work starts.",
    ...
    "delivery.status.decision.proposed": "Approve, change, or reject the limits for this one delivery before work starts.",
    ...
    "delivery.status.decision.terminal": "Decide a new one-time working choice before any further pull request or local release.",
  ...
  it: Object.freeze({
    ...
    "delivery.protection.common": "Distribuzione, accesso alla produzione, segreti, modifiche fuori dai file approvati e qualsiasi pull request o rilascio locale successivo richiedono una decisione separata.",
    ...
    "delivery.proposal.decision": "Approva o modifica questa scelta valida per una sola consegna dopo aver controllato progetto o cartella locale, destinazione, file modificabili, momenti di revisione e scadenza.",
    "delivery.proposal.next": "Se i limiti sono corretti, approva questa scelta. Altrimenti modificali prima di iniziare il lavoro.",
    ...
    "delivery.status.decision.proposed": "Approva, modifica o rifiuta i limiti per questa sola consegna prima di iniziare il lavoro.",
    ...
    "delivery.status.decision.terminal": "Decidi una nuova scelta valida una sola volta prima di un’altra pull request o rilascio locale.",
...
export function createHumanGuidance({ locale = "en", messages = {} } = {}) {
  ...
  const deliveryAutonomyProposal = (input = {}) => {
    ...
    const workingImpact = context.narrowed
    ...
    return buildHumanGuidance({
      ...
      required_decision: `${t("delivery.proposal.decision")} ${deliveryProposalReview(input, t, normalizedLocale)} ${deliveryProposalExpiry(input, context, t, normalizedLocale)}`,
  ...
  const deliveryAutonomyStatus = (input = {}) => {
    ...
    return buildHumanGuidance({
      ...
      required_decision: t(`delivery.status.decision.${decisionGroup}`),
      protection_boundary: deliveryProtection(t, context),
