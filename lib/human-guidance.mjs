  ...
  const actionCheckpoint = (input = {}) => {
    ...
    const localReleaseBoundary = action === "release.local"
      ? t("checkpoint.release.exact", {
          ...
          smoke: humanList(
            stringList(input.smoke_tests ?? input.local_release_target?.smoke_tests),
            normalizedLocale,
      ...
      : ["data.migrate", "data.rollback"].includes(action)
        ? t("checkpoint.data.exact", {
            ...
            scope: humanList(stringList(input.data_scopes), normalizedLocale)
            ...
            preview: humanList(stringList(input.preview_evidence), normalizedLocale)
        ...
        : action === "rollback.verify"
          ? t("checkpoint.rollback.exact", {
              ...
              evidence: humanList(stringList(input.rollback_evidence), normalizedLocale)
                || (normalizedLocale === "it" ? "le prove mostrate" : "the displayed evidence"),
            })
      : null;
