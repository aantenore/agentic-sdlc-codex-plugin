...
Agentic SDLC 0.8.1 is a self-contained Codex plugin. The repository root is the plugin root because it contains `.codex-plugin/plugin.json`; all manifest and agent-card paths are repository-relative.
...
| --- | --- | --- |
| `codex plugin list --available --json` | Installed entry is enabled and reports `0.8.1` | Rerun staging, add again, then open a new task |
